...
class SingupPage {
    ...
    fillForm(deliver) {
        ...
        cy.get(el.adressNumber).type(deliver.endereco.numero);
        cy.get( el.adressDetails).type(deliver.endereco.complemento);

        cy.get(el.adress).should('have.value', deliver.endereco.rua);
        cy.get(el.District).should('have.value', deliver.endereco.bairro);
        cy.get(el.uf).should('have.value', deliver.endereco.cidadeUF);
